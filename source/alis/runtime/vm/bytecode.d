...
/// Alis ByteCode
public struct ABC{
	string[] labelNames; /// labelNames, index corresponds `ABC.labels`
...
private:
	/// indices where labels occur in abc.code
	size_t[] _labOff;
	...
	/// label counter
	size_t _labC;
	/// dynamic data indices in abc.code
	size_t[] _dOff;
public:
	...
	void lPush(string label){
		_abc.labelNames ~= label;
		_abc.labels ~= _abc.code.length;
	}
	...
	/// commits and generates a final ABC
	/// Returns: ABC
	ABC commit(){
		ABC ret;
		ret.labelNames = _abc.labelNames.dup;
		ret.labels = _abc.labels.dup;
		ret.code = _abc.code.dup;
...
unittest{
	...
	writer.iPush!labTest("testLabel".Lab);
	writer.lPush("testLabel");
	code = writer.commit;

	import alis.runtime.vm : execute;
	execute!IS(code);
	assert (x == (1 | 2 | 4 | 8));
}
