...
class BytecodeGenerator {
	...
	private void generateDoWhileBytecode(RDoWhile doWhileStmt) {
		string startLabel = "dowhile_start_" ~ labelCounter.to!string;
		string endLabel = "dowhile_end_" ~ labelCounter.to!string;
		labelCounter++;

		addInstruction(startLabel, []);
		generateStatementBytecode(doWhileStmt.body);
		generateExpressionBytecode(doWhileStmt.condition);
		addInstruction("\tjmpC", ["@" ~ startLabel]); // Jump back if condition is true
		addInstruction(endLabel, []);
	}
...
unittest {
    ...
    */
    ...
    RDoWhile doWhileStmt = new RDoWhile;
    RIdentExpr conditionExpr = new RIdentExpr;
    conditionExpr.ident = "condition";
    doWhileStmt.condition = conditionExpr;

    RBlock bodyBlock = new RBlock;
    doWhileStmt.body = bodyBlock;

    RAssignExpr assignStmt = new RAssignExpr;
    RIdentExpr lhsExpr = new RIdentExpr;
    lhsExpr.ident = "result";
    assignStmt.lhs = lhsExpr;
    RLiteralExpr rhsExpr = new RLiteralExpr;
    rhsExpr.type = ADataType.ofInt;
    rhsExpr.value = asBytes!int(42);
    ...
    generator.generateDoWhileBytecode(doWhileStmt);
    printBytecode(generator.bytecodeInstructions, "do-while statement");
}
